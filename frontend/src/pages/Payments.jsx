        ...
        <div className="stat-card">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-white mt-1">{fmt(summary.total_earnings)}</p>
            </div>
        ...
        <div className="stat-card">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-white mt-1">{fmt(summary.pending_payouts)}</p>
            </div>
        ...
        <div className="stat-card">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-white mt-1">{fmt(summary.total_earnings - summary.pending_payouts)}</p>
            </div>
            ...
            <tbody>
              {loading ? Array.from({length:8}).map((_,i) => (
                <tr key={i} className="border-b border-gray-800">
                  {Array.from({length:7}).map((_,j) => <td key={j} className="px-4 py-3"><div className="h-4 bg-gray-800 rounded animate-pulse"/></td>)}
                </tr>
              )) : payments.length === 0 ? (
                ...
                <tr key={`${p.account_id}-${p.payment_id ?? i}`} className="table-row">
                  ...
                  <td className="px-4 py-3 text-gray-300 text-xs">{p.account_name}</td>
                  <td className="px-4 py-3 text-gray-400 text-xs">{p.order_count ?? '—'}</td>
                  <td className="px-4 py-3 text-green-400 font-medium">{fmt(p.amount)}</td>
                  <td className="px-4 py-3">
                    <span className={`badge ${statusStyle[p.status] || 'bg-gray-700 text-gray-400'}`}>{p.status || 'Unknown'}</span>
                  </td>
